...
const ADdCustomers=({custid})=> {
  const router = useRouter()
    const fetcher=async()=>{
        const response = await axios.get(`https://codingmart-invoice.herokuapp.com/invoicesbyid/${custid}`)
   ...
   const [value, setValue] = React.useState('1');
    ...
    const {data,error} = useSWR("custvsssiew",fetcher)
    console.log(data)
    const handleDelete = () =>{
      axios.delete(`https://codingmart-invoice.herokuapp.com/invoices/${data._id}`).
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Grid container spacing={0}>
            <table style={{width:"90%", border: "1px solid grey",borderCollapse: "collapse"}}>
                ...
                Tamil nadu <br/> 
                India</td>
                <th><h2 style={{padding:"10px",textAlign:"right"}}>TAX INVOICE</h2></th>
              </tr>
                ...
                <td style={{padding:"15px"}}>
                  ...
                  <td style={{padding:"15px"}}>
                  :  INV-000001<br/>
                  :  {data&&data.invoice_date}<br/>
                  :  {data&&data.terms}<br/>
                  :  {data&&data.due_date}<br/>
                  :  {data&&data.order_no}
                  </td>
              ...
              <tr style={{border: "1px solid grey",borderCollapse: "collapse",color:"#5c6f80"}} >
                <td style={{padding:"10px"}}><b>{data&&data.customer_name}</b></td>
              </tr>
